...
if __name__ == '__main__':
	...
	# setup data#{{{
	# weights, lengths = tvbhpc.load_connectome(args.dataset)
	# logger.info('weights.shape %s', weights.shape)
	# # logger.info('weights %f', weights)
	# logger.info('lengths.shape %s', lengths.shape)
	# # logger.info('lengths %f', lengths)
	# n_nodes = weights.shape[0]
	# logger.info('n_nodes %d', n_nodes)	

	#setup data
	weights = tvbhpc.connectivity.weights
	logger.info('weightsR.shape %s', weights.shape)
	lengths = tvbhpc.connectivity.tract_lengths
	logger.info('lengthsR.shape %s', lengths.shape)
	n_nodes = weights.shape[0]
	logger.info('n_nodes %d', n_nodes)	
	...
	def regular():
		...
		r = tvbhpc.plot_SC_FC(tvbhpc.SC, FC,"regular")
		print(r)
