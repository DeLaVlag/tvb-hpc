...
% endfor\
    ...
    // regular constants
% for item in const:
    __shared__ const float ${item.name} = ${item.default}
% endfor

    // coupling constants
% for m in range(len(coupling)):
    % for cc in (coupling[m].constants):
    __shared__ const float ${cc.name} = ${cc.default}
    %endfor
% endfor
        ...
        for (unsigned int i_node = threadIdx.y; i_node < n_node; i_node+=blockDim.y)
        {
            ...
            for (unsigned int j_node = 0; j_node < n_node; j_node++)
            {
                ...
                % for m in range(len(coupling)):
                    ...
                    % if coupling[m].dynamics.derived_variables and \
                        (coupling[m].dynamics.derived_variables['pre'].expression != 'None' or \
                        coupling[m].dynamics.derived_variables['post'].expression != 'None'):
                    % for cdp in (coupling[m].derived_parameters):

                ${cdp.name} += ${coupling[m].dynamics.derived_variables['post'].expression} * ${coupling[m].dynamics.derived_variables['pre'].expression}
